...
def generate_load_names():
  return [d for d in os.listdir(load_path)
...
class TestParser:
  ...
  def test_load(self, name):
    ...
    if os.path.exists(load_out_path):
      with open(load_out_path, 'r', encoding = 'utf-8') as f:
        expected = item_from_object(json.load(f))
      with open(load_in_path, 'r', encoding = 'utf-8') as f:
        result = load_naked(f)
      assert result == expected, f"Got different result. expected: {expected}, got: {result}"
    elif os.path.exists(load_err_path):
      with open(load_err_path, 'r', encoding = 'utf-8') as f:
        expected_err = json.load(f)
      with pytest.raises(FractalTextParseError) as err:
        with open(load_in_path, 'r', encoding = 'utf-8') as f:
          load_naked(f)
      assert err.value.line_no == expected_err['line'], f"Got an error in different location. expected: {expected_err.line}, got: {err.line_no}"
    else:
